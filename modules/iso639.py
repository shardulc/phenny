...
def iso639(phenny, input):
    """.iso639 <lg> | .iso639 <Language> - Search ISO 639-2 and -3 for a language code."""
    response = ""
    thisCode = str(input.group(2))
    if thisCode == "None":
        thisCode = random.choice(list(ISOcodes.keys()))
        #ISOcodes[random.randint(0,len(ISOcodes)-1)]
        #random.choice(ISOcodes)
    if thisCode in ISOcodes:
        response = template % (thisCode, ISOcodes[thisCode])
    else:
        if len(thisCode) > 3:      # so that we don't get e.g. 'a'
            for oneCode, oneLang in ISOcodes.items():
